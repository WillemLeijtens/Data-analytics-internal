...
# ------------------------------------------- als de naam niets prijsgeeft

def test_onduidelijke_naam_vraagt_om_een_keuze(client):
    """Twee retailers leveren dit formaat. Gokken zou de omzet van het ene
    land bij het andere zetten — dus vraagt de app het."""
    r = client.post("/api/import/controle", files=[
...
def test_een_keuze_buiten_de_kandidaten_telt_niet(client):
    ...
    r = client.post("/api/import", files=[("files", ("rapport.xlsx", _rapport(BE_WINKELS)))],
